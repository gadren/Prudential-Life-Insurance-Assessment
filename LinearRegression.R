prudent_train<-read.csv("C:\\Users\\Y Li\\Desktop\\train.csv\\train.csv")
prudent_test<-read.csv("C:\\Users\\Y Li\\Desktop\\train.csv\\test.csv")
str(prudent_train)
...
final_train_pca_data <- prcomp(training_df)
summary(final_train_pca_data)

#Pick 95% of variance of data using PCA
final_train_pca_data_trimmed <- data.frame(final_train_pca_data$x[,1:1070])

#Append Response variable to the dataset
pcaYVarTrain <- data.frame(c(final_train_pca_data_trimmed, prudent_train[c("Response")]))

#Divide given dataset to test and train
LinearTrain <-pcaYVar[2:15764,]
LinearTest <- pcaYVar[15765:19765,]

#Applying Linear Regression
lm_model <-lm(Response ~., data =LinearTrain)

summary(lm_model)

#Predict Response Variable
pred <- predict(lm_model, LinearTest)

#Calculate accuracy
accuracy(pred,LinearTest$Response)

vector_id <- c(15765:19765)
LinearSolution <- data.frame(Id = vector_id, response = pred)
write.csv(LinearSolution, file = "Prudentail_Linear_Result.csv", row.names = FALSE)

#Plot response distribution for test and train
ResponseOnTest <- qplot(Response, data=LinearTest, geom="density", alpha=I(.5), fill="density",
      main="Distribution of Response on test data", xlab="Response", 
      ylab="Density")
ResponseOnTrain <- qplot(Response, data=LinearTrain, geom="density", alpha=I(.5), 
      main="Distribution of Response on train data", xlab="Response", 
      ylab="Density")

#Show the results in a same page
grid.newpage() 
pushViewport(viewport(layout = grid.layout(nrow = 1, ncol = 2))) 
print(ResponseOnTest, vp = viewport(layout.pos.row = 1, layout.pos.col = 1))
print(ResponseOnTrain, vp = viewport(layout.pos.row = 1, layout.pos.col = 2)) 