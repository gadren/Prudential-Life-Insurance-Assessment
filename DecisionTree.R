prudent_train<-read.csv("C:\\Users\\ninad\\Desktop\\ADS\\Midterm Project\\train.csv")
prudent_test<-read.csv("C:\\Users\\ninad\\Desktop\\ADS\\Midterm Project\\test.csv")
str(prudent_train)
...
ggplot(prudent_train,aes(x=Response))+geom_histogram(stat="count",fill=I("LightBlue"),col=I("black"),binwidth = 5,main="Histogram for Response")+theme_bw()
...
for (n in colnames(prudent_train[categorical_variables])) {
  prudent_train[[n]] <- as.factor(as.numeric(prudent_train[[n]]))
  prudent_test[[n]] <- as.factor(test[[n]])
}
...
train <- prudent_train
#train$Response <- as.integer(train$Response)
dim(train)
...
library(rpart.plot)
library(forecast)
indxTrain <- createDataPartition(y = train$Response,p = 0.8,list = FALSE)
# Split data
...
library(rattle)
rattle()
mytree <- rpart(Response ~ .,data = train_data, method = "anova", control=rpart.control(cp=0.01))
summary(mytree)
par(mfrow = c(1,2)) # two plots on one page
rsq.rpart(mytree)

fancyRpartPlot(mytree, uniform=TRUE, main="Regression Tree")

myprediction <- predict(mytree, test_data)

View(round(myprediction, digits = 0))

ModelMetrics::rmse(myprediction, test_data$Response)

pResult <- as.integer(round(myprediction, digits = 0))
View(pResult)



accuracy(pResult,test_data$Response)

acc<-which(train_data$Response - pResult == 0)
length(acc)/length(test_data$Response)*100
...
#Pruning the tree
#ptree <- prune(mytree, cp = mytree$cptable[which.min(mytree$cptable[,"xerror"]),"CP"])
#fancyRpartPlot(ptree, uniform=TRUE, main="Pruned Tree")

#mypredict <- as.integer(round(predict(ptree, test_data)))

#ModelMetrics::rmse(mypredict, test_data$Response)

#acc<-which(test_data$Response-mypredict==0)
#length(acc)/length(test_data$Response)*100

#accuracy(myprediction,test_data$Response)

#Trying to improve on accuracy
mytree1 <- rpart(Response ~ BMI + Medical_History_2 + Medical_History_4 + Medical_Keyword_3 + Product_Info_4,data = train_data, method = "anova", cp=0.01)
fancyRpartPlot(mytree1, uniform=TRUE, main="Regression Tree1")

myprediction1 <- predict(mytree1, test_data)

ModelMetrics::rmse(myprediction1, test_data$Response)

accuracy(myprediction1,test_data$Response)

mysolution <- data.frame(ID = test_data$Id, Prediction = pResult)
View(mysolution)
write.csv(mysolution, file ="C:/Users/ninad/Desktop/ADS/Midterm Project/Soltion_DT.csv", row.names = FALSE)